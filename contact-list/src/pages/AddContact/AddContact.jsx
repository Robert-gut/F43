        ...
        <Formik initialValues={initialValues} validationSchema={contactValidationSchema} onSubmit={handleSubmin}>
          {({ isSubmitting }) => (
            <Form>
              ...
              <div className="mb-4">
                ...
                <Field className="form-control fs-5" as="select" name="status">
                  ...
                  {Object.keys(contactStatuss).map((status) => (
                    <option style={{ background: contactStatuss[status].bg }} key={status} value={status}>
                      {status}
                    </option>
                  ))}
                </Field>
              ...
              <div className="mb-4">
                <label className="form-check-label fs-5" htmlFor="favorite">
                  Favorite
                </label>
                <Field className="form-check-input m-1 fs-4" type="checkbox" name="favorite" />
              </div>
              <button type="submit" className="btn btn-primary btn-lg form-control" disabled={isSubmitting}>
                Add
              </button>
            </Form>
          )}
